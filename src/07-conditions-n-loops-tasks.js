...
function getFactorial(n) {
  return Array.from({ length: n })
    .fill(0).map((item, index) => index + 1).reduce((acc, i) => acc * i);
}
...
function getSumBetweenNumbers(n1, n2) {
  return Array.from({ length: n2 - n1 + 1 })
    .fill(0).map((item, index) => n1 + index).reduce((acc, i) => acc + i);
}
...
function getDigitalRoot(num) {
  ...
  const sum = String(num).split('').reduce((acc, item) => acc + Number(item), 0);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < firstPath.length; i += 1) {
    if (arr.every((item) => firstPath[i] === item[i])) newArr.push(firstPath[i]);
  }
  if (newArr.length === 0) {
    return '';
  }
