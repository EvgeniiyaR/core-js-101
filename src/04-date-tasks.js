...
function timeSpanToString(startDate, endDate) {
  const h = endDate.getHours() - startDate.getHours();
  const m = endDate.getMinutes() - startDate.getMinutes();
  const s = endDate.getSeconds() - startDate.getSeconds();
...
function angleBetweenClockHands(date) {
  ...
  const normalHour = h > 12 ? h - 12 : h;
  const angleHour = 0.5 * (60 * normalHour + m);
  const angleMin = 6 * m;
  let diffAngle = Math.abs(angleHour - angleMin);
